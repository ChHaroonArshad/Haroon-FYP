...
import { Palette, Menu, X, Heart, User, LogOut, ChevronDown } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-16 md:h-20">
        ...
        <Link to="/" className="flex items-center gap-2 flex-shrink-0">
          <div className={`p-1.5 rounded-xl transition-colors ${
            isTransparent ? 'bg-white/20' : 'bg-purple-50'
          }`}>
        ...
        <div className="hidden md:flex items-center gap-2">
          {isLoggedIn ? (
              ...
              {user.role === 'buyer' && (
                <Link to="/buyer/favorites"
                  className={`p-2 rounded-lg transition ${
                    isTransparent ? 'text-white/80 hover:bg-white/15' : 'text-gray-500 hover:bg-gray-100'
                  }`}>
            ...
            <>
              <Link to="/login">
                <button className={`px-5 py-2 rounded-xl text-sm font-semibold border-2 transition ${
                  isTransparent
